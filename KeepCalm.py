...
def drawEmoji(info, location, target):
    
    if info['scores']['neutral'] > 0.5:
        return
    ...
    if best[0] == 'anger' or best[0] == 'contempt' or best[0] == 'disgust':
        emoji_raw = cv2.imread("angry_emoji.png", -1)
    elif best[0] == 'fear' or best[0] == 'surprise':
        emoji_raw = cv2.imread("fear_emoji.png", -1)
    elif best[0] == 'happiness':    
...
def drawInfo(frame):
    ...
    for index in range(0, min(len(tmp_response), len(face_locations))):
        drawEmoji(tmp_response[index], face_locations[index], frame)
